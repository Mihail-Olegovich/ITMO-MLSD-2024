...
На этапе пилота будем использовать существующую в компании инфраструктуру и локальные модели на собственных серверных решениях rtx/dgx. Оптимальная по качеству opensource модель на момент первоначальных экспериментов - qwen2.5-72b-instuct. 

Базовая формула для вычисления необходимого VRAM: Number of Parameters × (Precision / 8) × 1.2

Рассчитаем для нашей модели с учетом Precision = FP8: (72 * (8/8)) * 1.2 = 86,4 GB VRAM, что требует 1-2 карты NVIDIA H100 для инференса одной модели
...
| Количество драйверов сервиса                                                       | Время отклика на отчет не должно превышать 30 секунд при больших моделях, для остальных не более 15 секунд.<br>~3000 уникальных пользователей ежемесячно и ~ 50 единовременно |

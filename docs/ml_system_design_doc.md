...
Для проведения пилотного тестирования пользователями сервиса будет выполнена оценка результатов на основе различных метрик.

Мы будем использовать заранее подготовленные запросы с известными правильными результатами и эталонными SQL-запросами для их получения. Сгенерированные системой запросы будут сравниваться с эталонными. Одной из ключевых метрик станет execution rate, измеряющая долю успешно выполненных запросов.
...
Успешным пилот будет считаться, в случае, если на уровне запросов easy и medium, система будет показывать результаты на уровне 40-50% execution rate.
...
| Количество драйверов сервиса                                                       | Время отклика на отчет не должно превышать 5 секунд при больших моделях, для остальных не более 2 секунд.<br>~3000 уникальных пользователей ежемесячно и ~ 500 единовременно |

#### 4.4. Безопасность системы

Сервис обработки запросов должен выявлять атаки (контролировать целенаправленную нагрузку сервис)
